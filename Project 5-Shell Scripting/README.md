...
Basically, the idea of shell scripting is to automate and execute repeatitive tasks, for example, in a situation where we have to download a thousand different times from the web, this task can be executed using a script that will perform these repeatitive tasks.
Bash is a dialect of shell syntax called the Bourne shell. Bash scripts are usually a series of commands saved in a text file with a .sh extension
